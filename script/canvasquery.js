...
(function(window, undefined) {
    ...
    $.extend($, {
        ...
        blend: function(below, above, mode, mix) {
            ...
            var imageData = below.createImageData(below.canvas.width, below.canvas.height);
    ...
    $.Wrapper.prototype = {
        ...
        convolve: function(matrix, mix, divide) {
            ...
            var output = this.createImageData(this.canvas.width, this.canvas.height);
            var dst = output.data;
    ...
    $.Color.prototype = {
        ...
        blendOn: function(below, mode, mix) {
            below = $.color(below);
            ...
            if ($.specialBlendFunctions.indexOf(mode) !== -1) {
                ...
                var rgb = blendingFunction(this, below);

                this[0] = below[0] + (rgb[0] - below[0]) * mix;
                this[1] = below[1] + (rgb[1] - below[1]) * mix;
                this[2] = below[2] + (rgb[2] - below[2]) * mix;
                this[3] = below[3];

            } else {
                var r = blendingFunction(below[0], this[0]);
                var g = blendingFunction(below[1], this[1]);
                var b = blendingFunction(below[2], this[2]);

                this[0] = below[0] + (r - below[0]) * mix;
                this[1] = below[1] + (g - below[1]) * mix;
                this[2] = below[2] + (b - below[2]) * mix;
